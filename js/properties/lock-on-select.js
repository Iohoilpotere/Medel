...
export class LockOnSelectProperty extends BaseProperty{
  ...
  static appliesTo(el){
    ...
    return t==='checkbox' || t==='checkboxgroup' || t==='tablecheck';
  }
  render(value, onChange){
    const wrap = document.createElement('div'); wrap.className='prop-group';
    const row = document.createElement('div'); row.className='prop-row';
    const lab = document.createElement('label'); lab.textContent='Una volta selezionato non si può deselezionare'; row.appendChild(lab);
