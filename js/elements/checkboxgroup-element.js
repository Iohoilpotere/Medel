  ...
  updateDom(){
    ...
    if((this.getProp('selectionStyle')||'standard')==='pills'){
      ...
      items.forEach((txt,i)=>{
        ...
        const bc = this.getProp('pillBorderColor') || 'rgba(255,255,255,0.3)'; const bw = Number.isFinite(this.getProp('pillBorderWidth'))? this.getProp('pillBorderWidth'):1; b.style.border = String(bw*zoom)+'px solid '+bc;
        ...
        { const fam = (ov.fontFamily ?? this.getProp('fontFamily')) || 'System';
          ...
          b.style.fontSize   = (unit==='px'? String(fs*zoom)+'px' : String(fs)+unit);
          ...
          b.style.letterSpacing = (Number.isFinite(ls)? ls*zoom:0)+'px';
          b.style.lineHeight  = Number.isFinite(lh)? String(lh) : '1.2';
          if(sw>0 && sc){ b.style.webkitTextStroke=`${(sw*zoom)}px ${sc}`; try{ b.style.textStroke=`${sw}px ${sc}`; }catch(e){} } else { b.style.webkitTextStroke=''; try{ b.style.textStroke=''; }catch(e){} }
          b.style.textShadow = shOn? `${(dx*zoom)}px ${(dy*zoom)}px ${(bl*zoom)}px ${shC}` : '';
        }
    ...
    // __ALIGN_NON_PILLS__ – align input/label for non-pill mode only
    (()=>{
      try{
