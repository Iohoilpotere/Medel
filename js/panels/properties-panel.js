...
export class PropertiesPanel{
  ...
  renderFlagProperties(flag){
    const store = this.app && this.app.flags;
    this.form.innerHTML = '';
    const grp = document.createElement('div'); grp.className='prop-group';
    const h = document.createElement('h4'); h.textContent='Flag'; grp.appendChild(h);
    ...
    (function(){
      ...
      const l=document.createElement('label'); l.textContent='Visibile'; r.appendChild(l);
      const i=document.createElement('input'); i.type='checkbox'; i.checked = (flag.visible!==false);
      i.addEventListener('change', ()=>{ 
        const store = this.app && this.app.flags;
        store && store.update(flag.id, { visible: !!i.checked });
        this.app.indicatorsPanel?.render();
      });
    ...
    const addRow = (label, value, onChange, type='text')=>{
      ...
      if(type==='checkbox'){ i.checked = !!value; i.addEventListener('change', ()=> onChange(!!i.checked)); }
      else { i.value = value||''; i.addEventListener('input', ()=> onChange(i.value)); }
      r.appendChild(i); content.appendChild(r);
    };

    addRow('Nome', flag.name||'', v=>{ store.update(flag.id,{name:v}); this.app.indicatorsPanel?.render(); });
    addRow('Immagine (URL)', flag.image||'', v=>{ store.update(flag.id,{image:v}); this.app.indicatorsPanel?.render(); });
    // Optional initial value (off by default)
    addRow('Valore iniziale (ON)', !!flag.value, v=>{ store.update(flag.id,{value:!!v}); }, 'checkbox');
  ...
  renderIndicatorProperties(ind){
    ...
    (function(){
      const r=document.createElement('div'); r.className='prop-row';
      const l=document.createElement('label'); l.textContent='Visibile'; r.appendChild(l);
      const i=document.createElement('input'); i.type='checkbox'; i.checked = (ind.visible!==false);
      i.addEventListener('change', ()=>{ 
        const store = this.app && this.app.indicators;
        store && store.update(ind.id, { visible: !!i.checked });
        this.app.indicatorsPanel?.render();
      });
      r.appendChild(i); content.appendChild(r);
    }).call(this);
  ...
  render(){ const step = this.app.currentStep; this.form.innerHTML=''; if(!step){ this.form.innerHTML='<div class="muted">Nessuno step selezionato</div>'; return; }
    ...
    if(targets.length===0){
      ...
      if(left==='indicators'){
        ...
        const selFlag = this.app && this.app.flags && this.app.flags.selected;
        if(selFlag){ this.renderFlagProperties(selFlag); return; }
        if(selInd){ this.renderIndicatorProperties(selInd); return; }
        this.form.innerHTML=''; return;
      ...
      (function(){
        ...
        const gU = document.createElement('div'); gU.className='prop-group';
        const hU = document.createElement('h4'); hU.textContent='Sblocco step'; gU.appendChild(hU);
        const cU = document.createElement('div'); cU.className='prop-group-content'; gU.appendChild(cU);

        const rowSel = document.createElement('div'); rowSel.className='prop-row';
        const labSel = document.createElement('label'); labSel.textContent='Criterio'; rowSel.appendChild(labSel);
        const sel = document.createElement('select');
        const options = [
          {v:'confirmPrev', t:'Conferma il precedente'},
          {v:'viewPrev', t:'Visualizza il precedente'},
          {v:'indicatorGte', t:'Indicatore ≥'},
          {v:'indicatorLte', t:'Indicatore ≤'}
        ];
        options.forEach(o=>{ const opt=document.createElement('option'); opt.value=o.v; opt.textContent=o.t; sel.appendChild(opt); });
        sel.value = step.unlock.mode || 'viewPrev';
        rowSel.appendChild(sel); cU.appendChild(rowSel);

        const rowThr = document.createElement('div'); rowThr.className='prop-row';
        const labThr = document.createElement('label'); labThr.textContent='Soglia'; rowThr.appendChild(labThr);
        const inpThr = document.createElement('input'); inpThr.type='number'; inpThr.step='any'; inpThr.value = step.unlock.threshold != null ? step.unlock.threshold : 0;
        rowThr.appendChild(inpThr); cU.appendChild(rowThr);

        function toggleThreshold(){
          const m = sel.value;
          const needs = (m==='indicatorGte' || m==='indicatorLte');
          rowThr.style.display = needs ? '' : 'none';
        }
        toggleThreshold();

        sel.addEventListener('change', ()=>{ step.unlock.mode = sel.value; toggleThreshold(); });
        inpThr.addEventListener('input', ()=>{ const v=parseFloat(inpThr.value); step.unlock.threshold = isNaN(v)? 0 : v; });

        this.form.appendChild(gU);
      }).call(this);
      /* Unlock group injected */

      // === Blocca step su conferma ===
      (function(){
        const step = this.app.currentStep || {};
        const grp = document.createElement('div'); grp.className='prop-group';
        const h = document.createElement('h4'); h.textContent='Blocco modifica'; grp.appendChild(h);
        const c = document.createElement('div'); c.className='prop-group-content'; grp.appendChild(c);
        const row = document.createElement('div'); row.className='prop-row';
        const lab = document.createElement('label'); lab.textContent='Blocca step su conferma'; row.appendChild(lab);
        const inp = document.createElement('input'); inp.type='checkbox'; inp.checked = !!step.lockOnConfirmStep;
        inp.addEventListener('input', ()=>{ step.lockOnConfirmStep = !!inp.checked; });
        row.appendChild(inp); c.appendChild(row);
        this.form.appendChild(grp);
      }).call(this);
      /* Lock step on confirm injected */
    
    
      // === Pulsanti ===
      (function(){
        const step = this.app.currentStep || {};
        const g2 = document.createElement('div'); g2.className='prop-group';
        const h2 = document.createElement('h4'); h2.textContent='Pulsanti'; g2.appendChild(h2);
        const c2 = document.createElement('div'); c2.className='prop-group-content'; g2.appendChild(c2);
        const addRow = (label, value, onChange, type='text')=>{
          const r=document.createElement('div'); r.className='prop-row';
          const l=document.createElement('label'); l.textContent=label; r.appendChild(l);
          const i=document.createElement('input'); i.type=type; i.value = (value ?? ''); i.addEventListener('input',()=> onChange(i.type==='checkbox' ? i.checked : i.value)); 
          if(type==='checkbox'){ i.checked = !!value; }
          r.appendChild(i); c2.appendChild(r); return i;
        };
        // Ensure object
        step.buttons = step.buttons || {};
        const nextInput = addRow('Testo tasto Avanti', step.buttons.nextLabel ?? 'Avanti', (v)=>{ step.buttons.nextLabel = v; });
        const showConfirmInput = addRow('Mostra tasto Conferma', !!step.buttons.showConfirm, (v)=>{ step.buttons.showConfirm = v; toggleConfirmInput(); }, 'checkbox');
        const confirmInput = addRow('Testo tasto Conferma', step.buttons.confirmLabel ?? 'Conferma', (v)=>{ step.buttons.confirmLabel = v; });
        function toggleConfirmInput(){ 
          confirmInput.disabled = !showConfirmInput.checked;
        }
        toggleConfirmInput();
        this.form.appendChild(g2);
      }).call(this);
      /* Buttons group injected */
